...
;;          (t
;;           (jb-game-output
;;            (format "The lazy-zombie explains that paperwork is required to recieve a portal traveling permit.  Unfortunately, the only paperwork left is in the storage room.  The lazy-zombie goes on to say that he is too busy right now to retrieve it for you.  He snarkily remarks that you may watch the clock in the waiting room like everyone else."))))))
...
;;    (cond
;;     ((null person) :not-a-person)
;;     ((null askable-person) :not-askable-person)
;;     ((null valid-subject) :not-valid-subject)
...
(defun jb-game-process-words (words)
  (dispatch (words)
    ...
    (("help" "please help" "query" "instructions") jg-help-command)
    (("ask" "query" "question" "tell me about") jg-ask-command)
    ((("time-change") ("use") ("change" "the" "time") ("adjust" "the" "hand") ("adjust") ("move" "clockhand")) jg-time-change-command)
    ((("good" "bye") "bye" "good-bye") jg-goodbye-command)))
...
(defun relevant (words)
  (loop with irrelevant = '("the" "a" "that" "to" "on" "the")
        for word in words
        when (not (member word irrelevant))
        collect word))
  ...
  (let* ((player-id current-player-id)
    ...
    (cond ((equal result :thing-not-in-room)
          ...
          (t
           (set-attribute :things 32 :invisible nil)
           (jb-game-output
            (format "You have unlocked the %s.  There is paperwork inside.  You should pick this up and bring it to the lazy-zombie." thing-name))))))
