...
(defun jb-game-process-words (words)
  (dispatch (words)
    ...
    (("use" "throw" "bounce" "move" "turn" "activate") jg-use-command)
...
(defun jg-use-command (words)
  ...
  (let* ((player current-player-id)
         ...
         (thing-name (get-attribute :things thing-id :name))
         (pocket (getf player :pocket))
         (interactible (getf thing :interactible)))
    (cond ((equal result :interactible)
             (format "Using %s." (get-attribute :things thing-id :name)))
          ((jb-game-output "Thing %s is not interactible." thing-name))
          (not ((equal result :pocket))
               (jb-game-output "You do not have thing %s to use." thing-name))
          (t (get-attribute :things thing-id :interactible list-keys)))))
   ...
   (list :rooms (list
         ...
         :things (list
                  ...
                  (list :name "ball"
                        ...
                        :retrievable t
                        :interactible "You bounce the ball off the floor.  That was fun"
                        :combinable nil
                        :combined nil
                        :inside-item nil
                        :contains-item nil
                  ...
                  (list :name "sword"
                        ...
                        :retrievable t
                        :interactible "I know this thing isn't sharp, but you shouldn't swing it around willy nilly."
                        :combinable nil
                        :combined nil
                        :inside-item nil
                        :contains-item nil
                  ...
                  (list :name "zombie janitor"
                        ...
                        :combined nil
                        :inside-item nil
                        :contains-item (list :id 9 :name "small grey key")
                        :conversable nil
                        :creature)
                  (list :name "dead flashlight"
                        :id 12
                        :room 3
                  ...
                  (list :name "gnome"
                        ...
                        :combined nil
                        :inside-item nil
                        :contains-item (list :id 16 :name "ticket 1-A")
                        :conversable t
                        :creature t)
                  (list :name "ticket 7A"
                        :id 16
